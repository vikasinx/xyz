    ...
    */
    ...
    function add_customer_rating_menu_endpoints()
    {
        ...
        if ((isset($userCanAcessPage)) && (!in_array($current_user->roles[0], $userCanAcessPage))) {
            ...
            //$product_ids = WC_Product_Vendors_Utils::get_vendor_product_ids(); //WCPV

            if (is_user_logged_in()) {
                $user_id = get_current_user_id();
                if (wcfm_is_vendor($user_id) && ($user_id == $vendor_id)) {
            ...
            //$product_ids = WC_Product_Vendors_Utils::get_vendor_product_ids(); //WCPV
            $booking_ids = array();
            $booking_ids = apply_filters('wcfm_wcb_include_bookings', '');
            //$booking_ids = array();
            ...
            if ((!empty($_GET['bid'])) && (in_array($_GET['bid'], $booking_ids))) {
                ...
                if(!empty($booking_ids)) {
                    echo '<table id="wcfm-bookings" class="display dataTable dtr-inline" cellspacing="0" width="100%" role="grid" aria-describedby="wcfm-bookings_info" style="width: 100%;">
                            <thead>
                                <tr role="row">
                                    <th class="sorting_asc" rowspan="1" colspan="1" aria-label=""><span class="wcicon-status-processing text_tip" data-tip="Status" data-hasqtip="83"></span></th>
                                    <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Booking">Booking</th>
                                    <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Product">Product</th>
                                    <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Order">Order</th>
                                    <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Start Date">Start Date</th>
                                    <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="End Date">End Date</th>
                                    <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Rating">Ratings</th></tr>
                            </thead>
                            <tfoot>
                                <tr>
                                    <th rowspan="1" colspan="1"><span class="wcicon-status-processing text_tip" data-tip="Status" data-hasqtip="84"></span></th>
                                    <th rowspan="1" colspan="1">Booking</th>
                                    <th rowspan="1" colspan="1">Product</th><th rowspan="1" colspan="1">Order</th>
                                    <th rowspan="1" colspan="1">Start Date</th><th rowspan="1" colspan="1">End Date</th>
                                    <th rowspan="1" colspan="1">Ratings</th></tr>
                            </tfoot>
                            <tbody>';
    ...
    {
        echo '<div class="error"><p>' . sprintf(esc_html__('Review Your Customer requires WooCommerce to be installed and active.', 'wc-review-your-customer')) . '</p></div>';
    }
